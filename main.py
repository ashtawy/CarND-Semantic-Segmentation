...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output = tf.layers.conv2d_transpose(output, num_classes, 4, strides=(2, 2), 
                                        ...
                                        kernel_initializer= tf.random_normal_initializer(stddev=1e-2), 
                                        kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add(output, tf.layers.conv2d(vgg_layer4_out, num_classes, 1, 
                                             padding='same',
                                             kernel_initializer= tf.random_normal_initializer(stddev=1e-2),
                                             kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3)))
    output = tf.layers.conv2d_transpose(output, num_classes, 4, strides=(2, 2),
                                        ...
                                        kernel_initializer= tf.random_normal_initializer(stddev=1e-2), 
                                        kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add(output, tf.layers.conv2d(vgg_layer3_out, num_classes, 1, 
                                             padding='same',
                                             kernel_initializer= tf.random_normal_initializer(stddev=1e-2),
                                             kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3)))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    train_hist_f = open('./runs/train_history.csv','w')
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            ...
            print('Cross entropy loss = %.3f'%(loss))
            train_hist_f.write('%d,%.4f\n'%(epoch+1, loss))
    train_hist_f.close()  
